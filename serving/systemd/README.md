...
| `taey-presence-engine.service` | prediction, interrupt, retrieval, and DCM presence loop | n/a |
...
The presence engine reads its site config from `@TAEY_ROOT@/presence-engine/.env`. That file is
optional at the systemd boundary so a missing file reaches the engine's explicit
`MODEL_ENDPOINT is required` failure instead of being misreported by systemd as a spawn-resource
error.
